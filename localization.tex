...
Localization was first introduced on $S^4$ for $\mN=2,4$ theories in \cite{Pestun:2007rz} and then applied to $\mathbb{S}^3$ and $\mathbb{S}_b^3$ for $\mN=2$ theories in \cite{Kapustin:2010mh} and \cite{Hama:2011ea}.\\
The basic principle about localization is the following.
Suppose that the theory in question admits a \emph{fermionic} symmetry $\delta$ which leave the measure of the path integral invariant.
Suppose that this transformation $\delta$ squares to a \emph{bosonic} symmetry such as a Lorentz or gauge transformation.\\
...
\end{equation}
where $V$ is a fermionic operator invariant under $\delta^2$.\\
...
The fields can be rescaled around these points by 
