...
We will start our analysis on electric-magnetic duality studying the first pair of theories that were discovered to be dual in \cite{Seiberg:1994pq}.  
...
Considering that the theory has no superpotential, the classical moduli space of the theory is given by \emph{D-terms} only. 
...
The lagrangian involving mesons features a non-trivial K\"{a}hler potential that reads
\begin{equation}
...
\paragraph{Pure SYM}
 For pure \emph{Super Yang Mills}, i.e. no quarks, the theory exhibit a discrete set of $N_c$ vacua. 
...
The gaugino condensation breaks R-symmetry to $\mathbb{Z}_2$ and in fact there are $N_c$ physically different vacua labeled by different phases of the gaugino condensate.
\paragraph{$\mathbf{ N_f < N_c}$}
The quantum corrections for \emph{SQCD} with $N_f < N_c$ flavours completely lift the moduli space through the Affleck-Dine-Seiberg (ADS) superpotential \cite{Davis:1983mz,Affleck:1983mk} which reads
\begin{equation}
...
\caption{Qualitative graph of the ADS superpotential }
\end{figure}
This situation is the perfect example where, unlike the classical moduli space, quantum corrections lift completely the moduli space and the theory does not posses a vacuum anymore.
...
\\
\begin{SCfigure}[1.6][b]
\centering
{\includegraphics[width=10cm]{terning.png} }
{\caption{ Classical and quantum 2D slice of the moduli space near the origin for $N_f = N_c$ \cite{Terning}} }
\end{SCfigure}
...
\end{equation}
where the bound is saturated for chiral fields.\\
...
Its behavior is not well defined at high energies because of the presence of a Landau pole at $R \sim \Lambda^{-1}$, although the theory can be a good description of a low-energy limit of another theory. 
