...
\\
\\
$\mathbf{  \frac{3}{2} N_c \geq N_f \geq  3 N_c}$\\
In this range the theory is not asympotically free. This can be seen by using the \emph{NSVZ} $\beta$ function,using \ref{eqn:suN_Dynkin_indices}, reads
\begin{align}
...
\end{align} 
Even though the $\beta$ function is exact, the anomalous dimension is only known perturbatively.\\
...
Its behaviour is not very well defined at high energies because of the presence of a Landau pole at $R \sim \Lambda^{-1}$, although the theory can be a very good description of a low energy limit of another theory. 
...
In the case of Seiberg duality, the electric theory is given by the theory we analysed previously i.e. $SU(N) \;SQCD$ with $N_f$ flavours. 

\subsubsection{Msagnetic theory}
The magnetic theory is \emph{SQCD} theory with the same global symmetries as the electric theory, but with gauge group $SU(N_f - N_c)$. 
In addition there are $N_f^2$ chargeless fields, that we will call mesons, since they have the same properties as the mesons we can construct in the electric theory.
...
In principle, mesons constructed out of the dual quarks could enter in the superpotential but it's easy to see that this is equivalent to a shift in $M$.\\
The charges of fields of the magnetic theory are
\begin{table}[h]
 \begin{tabular}{c | c |  c c c c }
 & $SU(N_c) $& $SU(N_f)_L$  &$SU(N_f)_R $  & $U(1)_B$ &  $U(1)_R$ \\
\hline
$q$ & $N_c$ & $N_F$ & $1$   &  $ \frac{N_c}{N_f-N_c} $  & $ \frac{N_c}{N_f}$  \\
$\tilde{q}$ &$\overline{N_c} $ &  $1$ & $\overline{ N_F}$   & $- \frac{N_c}{N_f-N_c}$   &  $ \frac{N_c}{N_f}$   \\	 
$M^i_{\tilde{j}} $ & $N_f$ & $\overline{N_f}$ & $\overline{N_f} $ & $0$ & $ 2 \frac{N_f - N_c}{N_f} $
 \end{tabular}
	\centering
...
This condition will grant that they are the same of the superconformal algebra.\\
Mesons in the magnetic theory have the same charges of the mesons constructed from electric quarks.\\
Using these charges the 't Hooft anomaly matching conditions are satisfied.\\
...
In the \emph{conformal window} ($\frac{3}{2} N_c \geq N_f \geq  3 N_c$) the electric and magnetic theories we introduced previously give an equivalent description of the same physics in the infrared.
In this range, the magnetic theory has a non trivial infrared fixed point too. 
At this fixed point, the superpotential \ref{eqn:seiberg_duality_mag_super} is a relevant perturbation since it has dimension $D = 1 + 3 \frac{N_c}{N_f}  <3 $ and it drives the theory in a new fixed point.\\
...
\begin{equation}
 \Lambda^{3 N_c - N_f} \tilde{\L}^{3 (N_f - N_c) - N_f} = (-1)^{N_f - N_c} \mu^{N_f}
...
Meson fields are massive and can be integrated out by using their equation of motion that result in
\begin{equation}
...
A mapping should exists also for baryonic operators. It does exists and it is given by
\begin{align}
...
\end{align}
using \ref{eqn:seib_dual_matching_scales} it can shown that these mappings preserve the $\mathbb{Z}_2$ character of the duality. \\
As an additional check of the duality 't Hooft anomaly matching conditions have been calculated  in \cite{Seiberg:1994pq} for the electric and magnetic theories for the various global symmetries and they are give by
\begin{align}
SU(N_f)^3 \qquad & N_c \\
U(1)_R\, SU(N_f)^2 \qquad & -\frac{N_c^2}{2 N_f} \\
U(1)_B\, SU(N_f)^2 \qquad & \frac{N_c}{2} \\
U(1)_R \qquad & -N_c^2 - 1 \\
U(1)_R^3 \qquad & -N_c^2 - 1 - 2 \frac{N_c^4}{N_f^2} \\
U(1)_R^2 U(1)_B \qquad & - 2 N_c^2 \\
\end{align}
