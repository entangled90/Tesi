...
\end{equation}
The axial symmetry gives rise to an anomaly through the triangular graph $U(1)_A SU(N_c)^2$, hence it is not a symmetry of the theory.\\ 
...
\end{figure}
Every fermion in the theory contributes to the anomaly which, as a result, is proportional to the R-charge of the fermions running in the loop and the Dynkin index of its representation.
...
For this reason it is necessary to introduce a dimensionful scale $\mu$ in order to match their dimension in the UV: $ M = \mu M_m$, where $M_m$ are the magnetic mesons.\\
