...
As a result, the classical constraints on mesons and baryons are satisfied quantum mechanically.
...
In this range the theory is not asympotically free. This can be seen by using the \emph{NSVZ} $\beta$ function, which, using \ref{eqn:suN_Dynkin_indices}, reads
\begin{align}
...
This condition will grant that its the same R-symmetry present in the superconformal algebra.\\
...
\end{align}
using \ref{eqn:seib_dual_matching_scales} it can be shown that these mappings preserve the $\mathbb{Z}_2$ character of the duality. 
