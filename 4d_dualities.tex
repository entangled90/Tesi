...
\subsection*{Seiberg Duality --- $SUN(N)$ SQCD with $N_f$ flavourssis }
...
The gauge group breaks through the super Higgs mechanism, where every broken generator gets absorbed by the vector superfield in order to make a massive vector superfield.   
...
\end{equation}
where $\Lambda$ is the dynamically generated scale of the theory.
...
The quantum corrections for \emph{SQCD} with $N_f \le N_c$ flavours completely lift the moduli space through the Affleck-Dine-Seiberg (ADS) superpotential (\cite{Davis:1983mz}\cite{Affleck:1983mk}) which reads
\begin{equation}
...
\begin{equation}
 \Det{M} - B \tilde{B} = 0
\end{equation}
in the quantum corrected moduli space mesons and baryons satisfy
\begin{equation}
 \Det{M} - B \tilde{B} = \Lambda^{2 N_c}
\end{equation}
which flows to the classical constraint in the classical limit ($\Lambda \; \rightarrow \;0$).
The effect of this relation is that the origin  does not belong to the moduli space anymore and is smoothed out
Moreover, if $B$ or $\tilde{B}$ is zero, the meson matrix has full rank.
