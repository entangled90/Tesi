%!TEX root = tesi.tex
\documentclass[a4paper,titlepage,twoside,openright,12pt]{book}
\usepackage[british]{babel}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{amstext}
\usepackage{amssymb}
\usepackage{amsthm}
%\usepackage{fullpage}
%%\usepackage{mathrsfs}
\usepackage[utf8]{inputenc}
\usepackage{xcolor}
%%\usepackage{natbib}
\usepackage{listings}
\usepackage{color}
\usepackage{verbatim}
\usepackage[T1]{fontenc}
\usepackage{ color}
\usepackage{lmodern}
\usepackage[toc,page]{appendix}
\usepackage[outercaption]{sidecap}
%\usepackage{floatrow}
\usepackage{graphicx}
\usepackage{color}
\usepackage{url}
%\usepackage{makeidx}
\usepackage{hyperref}
\usepackage{titleps}
\hypersetup{colorlinks,
            citecolor=blue,
            filecolor=black,
            linkcolor=black,
            urlcolor=blue}
\usepackage[a4paper,%
bindingoffset=0.6cm%,%
%showframe%
]{geometry}
\usepackage{microtype}
%\usepackage[nottoc,numbib]{tocbibind}
%\usepackage[sans]{frontespizio}
%\usepackage{titlesec}
%\\usepackage{fancyhdr}
\usepackage{emptypage}
%\usepackage{graphicx}
\usepackage{transparent}
\usepackage{eso-pic}
\geometry{
	top=3.5cm,bottom=3.5cm
}
%\usepackage{titlesec}
%Interlinea a 1 --> 1.5
%\linespread{1.5}
%\raggedbottom

%\usepackage[comma]{natbib}
\definecolor{gray75}{gray}{0.75}
\newcommand{\hsp}{\hspace{20pt}}
%\titleformat{\chapter}[hang]{\Huge\bfseries}{\thechapter\hsp\textcolor{gray75}{|}\hsp}{0pt}{\Huge\bfseries}
\newcommand{\HRule}{\rule{\linewidth}{0.5mm}} % Defines a new command for the horizontal lines, change thickness here
%Per avere il titolo dei capitoli con un box %Bjornstrup
\usepackage[ PetersLenny ]{ fncychap }
\ChNameVar{\Huge \bfseries \rm \color{\grey} }
\ChTitleVar{\Huge \bfseries \centering \color{\grey} }

\author{
Carlo Sana }
%Università degli studi di Milano-Bicocca \\
%Dipartimento di Fisica G. Occhialini}
\date{}
\title{4D to 3D reduction of Seiberg duality for $SU(N)$ susy gauge theories with adjoint matter: a partition function approach}


%%% Format titles of chapters

%\titleformat{\chapter}{\Huge \bfseries}{\thechapter}{1em}{}


%%%%%%%% TITLE PS!

 \newpagestyle{main}{
    \sethead[\thepage][\textmd{ \chaptertitle}][\thesection]  % even
            {\thesection}{\sectiontitle}{\thepage}
	\setheadrule{.55pt}%
            } % odd

 \newpagestyle{intro}{
    \sethead[\thepage][\textmd{ Introduction}][]  % even
            {\thesection}{Introduction}{\thepage}
	\setheadrule{.55pt}%
            } % odd

 \newpagestyle{biblio}{
    \sethead[\thepage][\textmd{ Bibliography}][]  % even
            {}{Bibliography}{\thepage}
	\setheadrule{.55pt}%
            } % odd

\newpagestyle{conclusion}{
    \sethead[\thepage][\textmd{ Conclusions}][]  % even
            {}{Conclusions}{\thepage}
	\setheadrule{.55pt}%
            } % odd
\newpagestyle{acknowledgment}{
    \sethead[\thepage][\textmd{ Acknowledgments}][]  % even
            {}{Acknowledgments}{\thepage}
	\setheadrule{.55pt}%
            } % odd

\newpagestyle{ringraziamenti}{
    \sethead[\thepage][\textmd{ Ringraziamenti}][]  % even
            {}{Ringraziamenti}{\thepage}
	\setheadrule{.55pt}%
            } % odd



\begin{comment}

%fancy
\setlength{\headheight}{15pt}
%\renewcommand{\chaptermark}[1]{\markboth{\small\ttfamily\chaptername\ \thechapter.\ #1}{}}
%\renewcommand{\sectionmark}[1]{\markright{\small\sffamily\thesection\ #1}}
\fancypagestyle{mypagestyle}{
	%%% FANCYHDR STUFF
	\pagestyle{fancy}
	\renewcommand\headrulewidth{1pt}%	\renewcommand\chaptermark[1]{\markboth{#1}{#1}}
	%\renewcommand\sectionmark[1]{\markright{#1}{#1}}
	\fancyhf{}
	\fancyhead[CE]{\normalsize \nouppercase{ \textmd {\leftmark}}}
	\fancyhead[CO]{\normalsize  \nouppercase{\textmd {\rightmark}}}
	\fancyhead[LE,RO]{\scshape \thepage}
}
%\fancyhead[L]{\scshape{Chapter \thechapter - \chaptertitle} }
%\fancyhead[R]{ \slshape  \nouppercase{\rightmark}}
%\fancyfoot[C]{\scshape \thepage}
%\fancyfoot[C]{\scshape \thepage}

\fancypagestyle{biblio}{
	\pagestyle{fancy}
	\renewcommand\headrulewidth{1pt}%	\renewcommand\chaptermark[1]{\markboth{#1}{#1}}
	%\renewcommand\sectionmark[1]{\markright{#1}{#1}}
	\fancyhf{}
	\fancyhead[CE]{\normalsize \nouppercase{\leftmark}}
	\fancyhead[CO]{\normalsize  \nouppercase{\rightmark}}
	\fancyhead[LE,RO]{\scshape \thepage}
}

\fancypagestyle{intro}{
	\pagestyle{fancy}
	%%% FANCYHDR STUFF
	\renewcommand\headrulewidth{1pt}%	\renewcommand\chaptermark[1]{\markboth{#1}{#1}}
	%\renewcommand\sectionmark[1]{\markright{#1}{#1}}
	\fancyhf{}
	\fancyhead[CE]{\normalsize Introduction }
	\fancyhead[CO]{\normalsize  Introduction}
	\fancyhead[LE,RO]{\scshape \thepage}
}

\fancypagestyle{conclusion}{
	\pagestyle{fancy}
	%%% FANCYHDR STUFF
	\renewcommand\headrulewidth{1pt}%	\renewcommand\chaptermark[1]{\markboth{#1}{#1}}
	%\renewcommand\sectionmark[1]{\markright{#1}{#1}}
	\fancyhf{}
	\fancyhead[CE]{\normalsize Conclusions }
	\fancyhead[CO]{\normalsize  Conclusions }
	\fancyhead[LE,RO]{\scshape \thepage}
}



\fancypagestyle{plainstyle}{
	\fancyhf{}
	\renewcommand\headrulewidth{0pt}%
}


\end{comment}

%%%%%%commands!!
%%% Comando per far le matrici con le barre verticali
\makeatletter
\renewcommand*\env@matrix[1][*\c@MaxMatrixCols c]{%
  \hskip -\arraycolsep
  \let\@ifnextchar\new@ifnextchar
  \array{#1}}
\makeatother

%%Comando per sottolineare testo con linea colorata in math mode
\newsavebox\MBox
\newcommand\Cline[2][red]{{\sbox\MBox{$#2$}%
  \rlap{\usebox\MBox}\color{#1}\rule[-1.2\dp\MBox]{\wd\MBox}{0.5pt}}}
  
%% comando per aligned nestati
\newlength{\myleftlen}
\newcommand{\setmyleftlen}[1]{\settowidth{\myleftlen}{\( \displaystyle
#1\)}}
\newcommand{\backup}{\hskip-\myleftlen\mkern-7mu}
  
  
  
%%comando per funzione segno
\newcommand{\sign}{\mbox{sign}}


%% Comando per N di supersymmetria

\newcommand{\mN}{\mathcal{N}}

%% Derivate parzielle
\newcommand{\dem}{\partial_{\mu}}
\newcommand{\deM}{\partial^{\mu}}
\newcommand{\den}{\partial_{\nu}}
\newcommand{\deN}{\partial^{\nsu}}

%Traccia e determinante
\newcommand{\Tr}{\mathrm{Tr } \,}
\newcommand{\Det}{\mathrm{det } \,}
\newcommand{\diag}{\hbox{diag\,}}

\newcommand{\til}[1]{\tilde{ #1}}


%ridefinisci overline più stretto
\newcommand{\overbar}[1]{\mkern 1.5mu\overline{\mkern-1.5mu#1\mkern-1.5mu}\mkern 1.5mu}



