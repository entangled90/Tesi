...
\\
{\huge commento R-symmetry e superconformal?}
When the superpotential \eqref{eqn:kutasov_superpotential_X} is relevant, the dimensions of the fields $X$,$Q,\tilde{Q}$ are given by the R-charges we found previously thanks to \eqref{eqn:superconformal_dimension_rcharge}.
\\
...
\begin{equation}
 Q_{(l)} = X^{l-1} Q \qquad l = 1, \dotsc, k
\end{equation}
Baryons are defined as
\begin{equation}
B^{i_1, i_2, \dotsc , i_k}) = Q_{(1)}^{i_1} \dots Q_{(k)}^{i_k} \qquad \hbox{with} \; \sum_{l=1}^k i_l = N_c 
\end{equation}
...
\end{equation}
Each $SU({i_l})$ sector describes a \emph{SQCD} model which has stable vacua only if $N_f \geq N_c$, hence considering every sector we have 
\begin{equation}
