...
\begin{align*}
c \left( 4 N_c  ( m  + m_B ) ( \omega ( N_f (1 - \Delta)  - N_c \Delta_X ) -  m_A N_f )\,  c( 4\big( \sum_{i=1}^{N_c}	 \sigma_i  \big) \big ( \omega( N_f (1 - \Delta)  - N_c \Delta_X)  -  m_A N_f ) \big) \right)\\
\end{align*}
...
\begin{align*}
 ...
 \frac{1}{ (2 \ \pi i)^{N_c} }{1 \over N_c ! } 
\Gamma_h ( \Delta_X \omega ; \omega_1 , \omega_2)^{N_c-1} \\
& c \big( 4 N_c  ( m  + m_B ) ( \omega ( N_f (1 - \Delta)  - N_c \Delta_X ) -  m_A N_f ) \big) \\
&\int_{T^{N_c-1}}  
...
\begin{aligned}
 ...
 \right) 
\end{align*}
Si può effettuare un ulteriore shift in $\sigma$:
\begin{equation}
	\tilde{\sigma} \; \longrightarrow \;  \tilde{\sigma }  -m_B \frac{N_c}{k N_f-N_c}  
\end{equation}
Con questo shift i quark si scaricano sotto  $U(1)_B$, ma si genera un termine di Chern-simons misto fra l'$U(1)_J$ e $U(1)_B$:
\begin{equation}
	e^{ \frac{\pi i }{ 2 \omega_1 \omega_2} 2 \xi \left( -m_B N_c   + \sum \tilde{\sigma} \right) }
\end{equation}
A questo punto le cariche si posson leggere direttamente dalla funzione di partizione e si può vedere che sono identiche alle cariche di \citep{Nii:2014jsa} (tenere conto che per me $j = 0, \dots ,k-1$, per lui $b_i$ partono da 1 (a caso!))
\begin{table} [hb]
\centering
\begin{tabular}{|c |c |c |c |c |c |c |}
\hline
 $ j =0, \dots , k-1$& $SU(N_f)_L$ & $SU(N_f)_R$ & $U(1)_B$ & $U(1)_A$ & $U(1)_J$ & $U(1)_R $ \\
\hline
 Electric Fields  & & & & & & \\
\hline
$Q$ & $N_f$ & 0  &$ N_c $& $1 $& 0  & $\Delta_Q$ \\
...
$X$ & 0 & 0 & 0 & 0 &0 & $\Delta_X$ \\ 
$M_j $ & $N_f$ & $\overline{N_f}$ & 0 & 0 & 0 & $2 \Delta_Q + j \Delta_X$\\ 
$b_j$ & 0  & 0 & 0  &  $ -N_f$ & $ \frac{1}{2}$   & $N_f ( 1 - \Delta_Q) + \Delta_X (j - N_c) $ \\
 $\tilde{b} _j$ & 0  & 0 & 0  &  $ - N_f$ & $ - \frac{1}{2}$   & $N_f ( 1 - \Delta_Q) + \Delta_X (j - N_c) $ \\
\hline
\end{tabular}
...
\subsubsection{$k_{BA}$}
Posso calcolare questo contact terms in entrambi i casi e il risultato è ovviamente uguale in entrambi e matcha con quello elettrico.
Il segno di $\mu_f$ è quello tale per cui il livello rimane positivo ( stessa convenzione  di antonio).
\begin{align*}
  &\mbox{Elettrico:}\\
  k^e_{BA} & = \frac{1}{2}  q^f_B q^f_A \, sign (\mu_{f})  \; = \; \frac{1}{2} N_f N_c \left( 1 \cdot 1 \, \sign (\mu) + 1 \cdot (-1) \sign ( - \mu) \right) \\
  & = N_f N_c \\
  & \mbox{Magnetico: primo caso}\\
  k^{m_1}_{BA} & =\frac{1}{2} 2 N_f (kN_f - N_c) \frac{N_c}{k N_f -N_c} = N_f N_c \\
  & \mbox{Magnetico: secondo caso}\\
  k^{m_2}_{BA} & = \frac{1}{2} ( 2  k ) \left( \frac{1}{k} \right) (N_f)  = N_f N_c 
\end{align*}
I livelli sono uguali in tutti i casi e infatti le fasi nella funzione di partizione sono identiche in tutti e tre i casi (e dovrei dimostrare che le fasi son generate da questi livelli: c'è un fattore di (-4) di differenza, è dovuto a come entra nell'azione. CONTROLLARE)

\subsubsection{$k_{BR}$}
Utilizziamo la stessa formula per calcolare questo livello, che dovrebbe essere identico in tutti e tre i casi (come prima).
\begin{align*}
 &\mbox{Elettrico:}\\
  k^e_{BR} & = \frac{1}{2}  q^f_B q^f_R \, sign (\mu_{f})  \; = \; \frac{1}{2} N_f N_c \left( 1 ( \Delta_Q - 1)  \, \sign (\mu) + -1 ( \Delta_Q - 1) \sign ( - \mu) \right) \\
  & = N_f N_c ( \Delta_Q - 1)  \\
  & \mbox{Magnetico: primo caso}\\
  k^{m_1}_{BR} & =\frac{1}{2} 2 N_f (kN_f - N_c) \frac{N_c}{k N_f -N_c} ( \Delta_X - \Delta_Q - 1) = N_f N_c ( \Delta_X - \Delta_Q - 1) \\
  & \mbox{Magnetico: secondo caso}\\
  k^{m_2}_{BR} & = \frac{1}{2} ( 2  k ) \left( \frac{1}{k} \right)  (\Delta_X - (-N_c \Delta_X + N_f ( 1-\Delta_Q)) =  N_c (\Delta_X - (-N_c \Delta_X + N_f ( 1-\Delta_Q))
\end{align*}


\subsubsection{$k_{BJ}$}
Non c'è modo di generare $k_{BJ}$ con quella formula, visto che nessun campo è carico sotto $U(1)_B$ e $U(1)_J$ insieme..
