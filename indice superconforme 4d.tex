...
& + \left(  -m_1 + m_2 + B + C + \sigma'_i - \rho'_j \right)^2 + \\
& - \left(  m_1 - m_2 - B - C - \sigma'_i + \rho'_j\right)^2 
\bigg)
\end{align*}
...
\bigg)
\end{align*}
La seconda riga è nulla in quanto è la differenza dello stesso quadrato. Per la prima riga invece abbiamo:
\begin{align*}
...
\mbox{Adj matter} & \begin{cases}
	+ m( 4 \omega ( \Delta_X -1) k ( k N_f - N_C) +\\
	- D ( 4 \omega ( \Delta_X -1) k ( kN_f - N_C) + \\
	- 4 k \sum_i \sigma'_i (  \omega ( \Delta_X -1) ) + \\
	+ 4 (k N_f - N_c)  \sum_i \rho'_i(  \omega ( \Delta_X -1) ) 
\end{cases}
\end{align*}
...
+ & ( \Delta_X -1) k ( k N_f - N_c)  \bigg) 
\end{align*}
Semplificando i vari termini usano il valore esplicito di $\Delta_X$ otteniamo:
\begin{align*}
...
\begin{align*}
 & m_A(- B N_f + C ) + \omega ( (C+B) ( -\Delta_X + 1)+( -k (k N_f -N_C) (\Delta_X -1) + B \Delta_M + C \Delta_Q) = \\
 & \omega \left( \frac{1}{k N_f - N_c} ( -\Delta_X + 1) ( 1 + k (k N_f - N_c)) + B \Delta_M + C \Delta_Q \right)+ \\
 & m_A \left(\frac{-N_c}{(k N_f - N_c) ( k (N_f+1) -N-c)} \right)  =\\
 & 
\end{align*}
...
\begin{align*}
  & 4 \sum_j^{k N_f - N_c} \sigma'_j \big( - m_A N_f -  \omega( \Delta_X - \Delta_M - 1) \big) +  4 N_f \sum_{j=1}^k  \rho'_j ( - m_A +  \omega ( \Delta_X - \Delta_Q - 1) ) =  \\
  = & 4  \sum_j^{k N_f - N_c} \sigma'_j \big( - m_A N_f -  \omega( \Delta_X - ( N_f (1 - \Delta_Q ) - N_c \Delta_X  + 1) - 1) \big) +\\
    +&  4 N_f \sum_{j=1}^k  \rho'_j ( - m_A +  \omega ( \Delta_X - \Delta_Q - 1) ) =  \\
    = & 4  \sum_j^{k N_f - N_c} \sigma'_j \big( - m_A N_f -  \omega(( N_c +1) \Delta_X - N_f (1 - \Delta_Q )-2) \big)  +\\
    +& 4 N_f \sum_{j=1}^k  \rho'_j ( - m_A +  \omega ( \Delta_X - \Delta_Q - 1) ) =
\end{align*}
...
Z_{m} ( \mu_i , \nu_j , \tilde{\mu_i}, \tilde{\nu_j} ) \,= & \, \frac{1}{ (k(N_f+1) - N_c)! (2 \pi i )^{k(N_f+1) - N_c}}
 ...
 &  c \left( 4m \omega ( k N_f - N_c)(-k)\right) \\
 & \bigg( \prod_a^{N_f+1 } \prod_b^{N_f+1}  \prod_{j=0}^{k-1} 
\Gamma_h \big( \mu_a+  \nu_b + j \omega \Delta_X) \big) \Gamma_h \big(- 2 m_A N_f +  \omega( \Cline[red]{\Delta_M} + j \Delta_X) \big)  \bigg) \\
&\int_{T^{\tilde{N}_c}}  \prod_{i=1}^{ k N_f - N_c } d \sigma_i \,  \prod_{i=1}^{ k } d \rho_i \, d \xi \, e^{2 \pi i \xi ( \sum \sigma_i + \sum \rho_i)}  \\
& \prod_{ 1 \leq i<j \leq k(N_f+1)- N_c } \frac{ \Gamma_h( \Delta_X \omega \pm (\sigma_i - \sigma_j)) }{ \Gamma_h ( \pm (\sigma_i - \sigma_j) )} \prod_{ 1 \leq i<j \leq k(N_f+1)- N_c } \frac{ \Gamma_h( \Delta_X \omega \pm (\rho_i - \rho_j)) }{ \Gamma_h ( \pm (\rho_i - \rho_j) )}\\
 &  \bigg( \prod_{a=1}^{N_f} \prod_{j=1}^{k N_f - N_c } \Gamma_h \big( m_B \frac{N_c}{k(N_f+1)-N_c} - m_a - m_A + \omega (\Delta_X - \Delta_Q)  + \tilde{\sigma}_j \big) \\
 & \Gamma_h \big(  -m_B \frac{N_c}{k(N_f+1)-N_c} + \tilde{m}_a - m_A + \omega (\Delta_X - \Delta_Q)  - \tilde{\sigma}_j\big) \bigg)\\
 & \bigg(\prod_{i=1}^{k} \Gamma_h \big( \pm(  \rho_i  + m_B \frac{N_c}{k(N_f+1)-N_c})+ m_A N_f + \omega( \Delta_X - \Cline[red]{\Delta_M }) \big) \bigg)\\
 &c (  4  \sum_j^{k N_f - N_c} \sigma_j \big( - m_A N_f -  \omega(( N_c +1) \Delta_X - N_f (1 - \Delta_Q )-2) \big) \\
 & c \big( 4 N_f \sum_{j=1}^k  \rho_j ( - m_A +  \omega ( \Delta_X - \Delta_Q - 1)  \big) 
 \end{align*}
