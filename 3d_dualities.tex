...
Moreover, in three dimensions spinors have only undotted indices, since the covering group of $SO(3)$ is $SU(2)$.
\\
In our notation the supercharges are complex, while minimal spinors in three dimensions are real, therefore resulting in $\mN=2$ theories.
...
It is given by
\begin{equation}
 \{ Q_{\alpha},Q_{\beta} \} =  \{ \bar{Q}_{\alpha},\bar{Q}_{\beta} \}= 0 \qquad \{ Q_{\alpha} , \bar{Q}_{\beta}   \} = 2 \gamma^{\mu}_{\alpha \beta} P_{\mu} + 2 i \epsilon_{\alpha \beta} Z
 \end{equation} 
 where the central charge $Z$ is the component of the momentum  along the reduced direction.\\
...
The quantization of the magnetic flux implies that the dualized photon is periodic and is normalized in such a way that $\gamma \sim \gamma + 2 \pi$.
\\
...
The possible windings around $S^2_{\infty}$ are then characterized by
\begin{equation}
...
Note that there are $N-1$ special embeddings of $SU(2)$ into $SU(N)$ which result in monopoles charged only under one factor of the $U(1)$ in the Cartan subalgebra. They are given by the N-1 contiguous $2 \times 2$ blocks in the diagonal.\\
...
The presence of the scalar field in the vector multiplet results in an additional branch of the moduli space which is called \emph{Coulomb branch}.
...
However in most dualities, the Coulomb branch will be completely lifted.\\
...
\begin{figure}[ht]
\center
\includegraphics[scale=0.5]{coulombranchu1.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.4]{moduliu1variousflavours.pdf}
...
As in four dimensions the Higgs branch is parametrized by mesons and, for $N_f \geq N_c$, by baryons too.
\\
...
\label{sec:kim-park-duality}
Aharony duality can be generalized with the addition of a field in the adjoint representation \cite{Kim:2013cma}.\\
