...
\vspace{0.3cm}
Il superpotenziale $\eta$ impone vincoli equivalenti all'assenza di \alert{anomalie in 4D}. \\
In 3D le anomalie non ci possono essere.
\\ Questi vincoli dovranno essere rimossi successivamente.
\\
...
\begin{frame}{Riduzione dimensionale in teoria di campo}
Si può dualizzare il settore $U(1)^k$  in modo da ottenere i singoletti non presenti in 4D.
\\
\vspace{0.5cm}

\alert{\bfseries Rimuovendo la deformazione $U(1)^k \rightarrow U(k)$ e non si sa come ottenere i singoletti della teoria magnetica.}
\onslide<2->{
\begin{center}
{\arrowdown}
\end{center}
...
Sfrutto la dualità fra le teorie in 4D, calcolando l'indice superconforme ( $I_{el} \; \& \; I_{mag} $).\\
Esso conta i multipletti corti BPS della teoria su $\mathbb{R}^3 \times \mathbb{S}^1$.\\
L'indice è dato da integrale sul gruppo di gauge di funzioni \alert{gamma ellittiche} $\Gamma_e$.\\
...
La funzione di partizione si ottiene usando identità matematiche che trasformano le \alert{gamma ellittiche} $\Gamma_e$ in \alert{gamma iperboliche} $\Gamma_h$

\begin{align*}
I_{el} & = I_{mag}   \qquad \qquad && \Gamma_e \\
\downarrow & \quad \downarrow  \qquad r \rightarrow 0 \qquad && \downarrow\\ 
Z^{\eta}_{el} & = Z^{\eta}_{mag}&& \Gamma_h
\end{align*}
\end{frame}
