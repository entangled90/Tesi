...
\end{aligned}
\end{equation}
Thus, the $U(k)$ sector is dual to a theory withouth gauge group and with $3k$ singlets, where $2k$ of them are charged under the topological symmetry. 
...
S=& \prod_{j=0}^{k-1} \Gamma_h \left( 2 m_A N_f + \omega( 2 -j \Delta_X  - 2 \Delta_M)\right) 
\end{aligned}
\end{equation}
...
\begin{equation}
  W = m \, M^{\dagger} M \quad \longrightarrow \quad R(M^{\dagger}) = 2 - R (M) \qquad Q(M^{\dagger}) = - Q(M)
\end{equation}
