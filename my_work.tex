...
\begin{equation}
 \xi \; \longrightarrow \; \frac{\xi}{2 \omega_1 \omega_2} \qquad 
 d \xi \; \longrightarrow \;  |2 \omega_1 \omega_2 | \, d \xi  = 
...
&\int  \prod_{i=1}^{ k N_f - N_c } \frac{d \sigma_i }{\sqrt{-\omega_1 \omega_2}}\, 
 \prod_{i=1}^{ k } \frac{d \rho_i}{\sqrt{-\omega_1 \omega_2}}
  \, \int |2 \omega_1 \omega_2 | d \xi \, e^{\frac{2 \pi i \xi}{\omega_1 \omega_2} ( \sum \sigma_i + \sum \rho_i)}  \\
%
& \prod_{ i<j }^{ k N_f - N_c } \frac{ \Gamma_h( \Delta_X \omega \pm (\sigma_i - \sigma_j)) }{ \Gamma_h ( \pm (\sigma_i - \sigma_j) )} \prod_{i<j }^{k } \frac{ \Gamma_h( \Delta_X \omega \pm (\rho_i - \rho_j)) }{ \Gamma_h ( \pm (\rho_i - \rho_j) )}\\
%
& \begin{aligned}
...
\begin{multline}
\prod_{j=0}^{k-1} \Gamma_h \left( \omega - j \omega \Delta_X - ( m_A N_f + \omega(\Delta_X - \Delta_M)) \pm \frac{\xi}{2} \right) 
\\
...
\end{aligned}
\end{equation}
We can use the mathematical identity $\Gamma_h (2 \omega - x) \Gamma_h(x) = 1$ which tells use that a chiral field with a holomorphic mass $m$ does not contribute to the partition function in the infrared.
