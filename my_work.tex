...
We can write down the formula for the partition function of the electric theory by putting al the pieces together and ignoring the divergente prefactor calculated previously.
...
\end{equation}
which in four dimension is equivalent to the condition that the R-symmetry is anomaly free.\\  

\subsection{Flow to a duality withouth $\eta$ superpotential}
With a suitable assignement of the real masses we can flow to a theory without $eta$ superpotential as we said before. 
Following \eqref{eqn:real_mass_reduction_el} the gauge group breaks  $ SU(N_f+1)^2 \times U(1)_B \, \rightarrow \, SU(N_f)_L \times SU(N_f)_R \times U(1)_A \times U(1)_B$. 
...
\mu_a = 
  \begin{cases}
    \omega \, \Delta + m_a + m_A + m_B \qquad  &a = 1, \dotsc, N_f\\ 
    \omega \, \Delta_M +  m - m_A N_f  + m_B \qquad  &a = N_f+1\\ 
  \end{cases}
  \\
\nu_b = 
  \begin{cases}
    \omega \, \Delta + \tilde{m}_b + m_A - m_B \qquad  &b = 1, \dotsc, N_f\\ 
