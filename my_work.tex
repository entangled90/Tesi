...
It is a $SU(\tilde{N}_c) = SU( k N_f - N_c)$ SQCD theory with $N_f$ flavours $q,\tilde{q} $, a matter field $Y$ in the adjoint representation and $k$ mesons $M$ that are identified with the electric mesons. There is a superpotential for the adjoint field $Y$ $ \Tr Y^{k+1}$ that results in $R_X = R_Y$\\
...
&I_{Mag} ( p,q,y,\tilde v,\tilde y) = \\
 ...
 \Gamma_e( (pq)^s;p,q)^{\tilde{N_c}-1} \bigg( \prod_a^{N_f} \prod_b^{N_f}  \prod_{l=0}^{k-1} \Gamma_e ( (pq)^{R_Q + l \frac{R_X}{2}} y_a \tilde{y_b}^{-1}; p ,q) \bigg) \\
&\int \bigg( \prod_{i=1}^{ \tilde{N_c }} { d z_i \over { 2 \pi i z_i} } \bigg) \delta \bigg( \prod_{i=1}^{\tilde{N_c}} z_i - 1 \bigg)
\prod_{ 1 \leq i < j \leq \tilde{N_c}} \frac{ \Gamma_e \big( (pq)^\frac{R_X}{2} {z_i \over z_j} \big) \Gamma_e \big( (pq)^\frac{R_X}{2} {z_j \over z_i} \big) }{ \Gamma_e( {z_i \over z_j};p,q) \Gamma_e({z_j \over z_i}; p,q)} \\
